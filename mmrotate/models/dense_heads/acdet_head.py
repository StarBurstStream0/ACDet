...
import torch
from mmcv.ops import DeformConv2d, ModulatedDeformConv2d, ModulatedDeformConvG
...
class AdapAlignModule_AGP(nn.Module):
    ...
    def _init_layers(self):
        ...
        self.sam_convs = nn.ModuleList()
        for i in range(self.conv_num):
            if i == 0:
                out_feat_num = self.feat_channels
            elif i == self.conv_num-1:
                out_feat_num = 18
            self.sam_convs.append(
                ConvModule(
                    self.feat_channels,
                    out_feat_num,
                    3,
        ...
        for _ in range(1):
            ...
            self.reg_convs.append(
                    DeformConv2d(
                        ...
                        bias=False))
        #############################################################
        ### TODO: add a hyper-parameter to achieve the multi-head cls
        self.group_num = 2
        self.cls_mdcn = nn.ModuleList()
        self.reg_mdcng = nn.ModuleList()
        for i in range(self.group_num):
            self.cls_mdcn.append(ModulatedDeformConv2d(
                        self.feat_channels,
                        self.feat_channels//self.group_num,
                        3,
                        stride=1,
                        padding=1,
                        groups=1,
                        bias=False))
            self.reg_mdcng.append(ModulatedDeformConvG(
                        self.feat_channels,
                        self.feat_channels//self.group_num,
                        3,
                        stride=1,
    ...
    def forward(self, cls_feat, reg_feat):
        sampling_dim = torch.Tensor(reg_feat.size(0), 1, reg_feat.size(-2), reg_feat.size(-1))
        stride = torch.ones_like(sampling_dim, device=reg_feat.device) * (self.dilation_rate - 1)
        zeros = torch.zeros_like(stride, device=reg_feat.device)
        sampling_priors = torch.cat((-stride,-stride, -stride,zeros, -stride,stride, \
                                zeros,-stride, zeros,zeros, zeros,stride, \
                                stride,-stride, stride,zeros, stride,stride), dim=1)

        for idx, sam_conv in enumerate(self.sam_convs):
            if idx == 0:
                sam_feat = sam_conv(reg_feat)
            else:
                sam_feat = sam_conv(sam_feat)
        sampling_priors += sam_feat
        reg_feat = self.reg_convs[2](reg_feat, sampling_priors)
        for cls_conv in self.cls_convs[:-2]:
            cls_feat = cls_conv(cls_feat)
        cls_feat = self.cls_convs[2](cls_feat, sampling_priors)

        reg_feats = []
        cls_feats = []
        offsets_reg = []
        mask_reg = []
        for reg_mdcng, cls_mdcn in zip(self.reg_mdcng, self.cls_mdcn):
            reg_feat_, offsets_reg_, mask_reg_ = reg_mdcng(reg_feat)
            cls_feat_ = cls_mdcn(cls_feat, offsets_reg_, mask_reg_)
            reg_feats.append(reg_feat_)
            cls_feats.append(cls_feat_)
            offsets_reg.append(offsets_reg_)
            mask_reg.append(mask_reg)
        reg_feat = torch.cat(reg_feats, dim=1)
        cls_feat = torch.cat(cls_feats, dim=1)
        offsets_reg = torch.stack(offsets_reg, dim=0)
        offsets_reg = offsets_reg.sum(dim=0) / self.group_num
        return cls_feat, reg_feat, offsets_reg

class AdapAlignModule(nn.Module):
    ...
    def forward(self, cls_feat, reg_feat):
        ...
        reg_feat = self.reg_convs[0](reg_feat, sampling_priors)
        reg_feat, offsets_reg, mask_reg = self.reg_convs[1](reg_feat)

        for cls_conv in self.cls_convs[:-2]:
            cls_feat = cls_conv(cls_feat)
        cls_feat = self.cls_convs[0](cls_feat, sampling_priors)
        cls_feat = self.cls_convs[1](cls_feat, offsets_reg, mask_reg)
        return cls_feat, reg_feat, offsets_reg

@ROTATED_HEADS.register_module()
class ACDetHead(RotatedAnchorHead):
    ...
    def __init__(self,
        ...
        self.norm_cfg = norm_cfg
        self.dilation_rate = dilation_rate
        self.with_agp = with_agp
        super(ACDetHead, self).__init__(
            num_classes,
            in_channels,
        ...
        for i in range(self.stacked_convs-2):
            ...
            self.reg_convs.append(
                ConvModule(
                    ...
                    norm_cfg=self.norm_cfg))
        if self.with_agp:
            self.aam = AdapAlignModule_AGP(self.feat_channels,
                            self.conv_cfg,
                            self.norm_cfg,
                            self.dilation_rate)
        else: 
            self.aam = AdapAlignModule(self.feat_channels,
                                        self.conv_cfg,
                                        self.norm_cfg,
                                        self.dilation_rate)
    ...
    def forward_single(self, x):
        ...
        cls_feat, reg_feat, offsets_reg = self.aam(cls_feat, reg_feat)
    ...
    def loss(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, deform_anchors_list) = cls_reg_targets
        num_total_samples = (
            num_total_pos + num_total_neg if self.sampling else num_total_pos)

        # change anchor to deform anchor
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]
        # concat_anchor_list = []
        # for i, _ in enumerate(deform_anchors_list):
        #     concat_anchor_list.append(torch.cat(deform_anchors_list[i]))
        all_anchor_list = images_to_levels(deform_anchors_list,
                                           num_level_anchors)
    ...
    def _get_targets_single(self,
        ...
        return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, sampling_result, deformable_anchors)
    ...
    def get_targets(self,
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         pos_inds_list, neg_inds_list, sampling_results_list, deform_anchors_list) = results[:8]
        ...
        res = (labels_list, label_weights_list, bbox_targets_list,
               bbox_weights_list, num_total_pos, num_total_neg, deform_anchors_list)
    ...
    def get_bboxes(self,
        ...
        for img_id, _ in enumerate(img_metas):
            ...
            mlvl_anchors_new = []
            for i in range(num_levels):
                offsets_ = offset_list[i].permute(1, 2, 0).view(-1, 18)
                slvl_anchors = mlvl_anchors[i]
                dy = torch.zeros(offsets_.size(0)).cuda()
                dx = torch.zeros(offsets_.size(0)).cuda()
                for i in range(9):
                    dy += offsets_[:, 2*i]/9
                    dx += offsets_[:, 2*i+1]/9
                slvl_anchors[...,0] = slvl_anchors[...,0]  + dx
                slvl_anchors[...,1] = slvl_anchors[...,1]  + dy 
                mlvl_anchors_new.append(slvl_anchors)
            cls_score_list = [
