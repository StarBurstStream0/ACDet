...
class self_ORPNHead(RotatedRPNHead):
    ...
    def _get_targets_single(self,
        ...
        anchors_obb = flat_anchors[inside_flags, :]
        
        # print('anchors: ', anchors.size())
        # print('anchors: ', anchors[:10])

        # gt_hbboxes = obb2xyxy(gt_bboxes, self.version)
        # anchors_hbb = obb2xyxy(anchors_obb, self.version)

        assign_result = self.assigner.assign(
            # anchors, gt_hbboxes, gt_bboxes_ignore,
            # anchors_hbb, gt_hbboxes, gt_bboxes_ignore,
            anchors_obb, gt_bboxes, gt_bboxes_ignore,
            None if self.sampling else gt_labels)
        sampling_result = self.sampler.sample(assign_result, anchors_obb,
                                              gt_bboxes)
        ...
        num_valid_anchors = anchors_obb.shape[0]
        bbox_targets = anchors_obb.new_zeros((anchors_obb.size(0), 5))
        bbox_weights = anchors_obb.new_zeros((anchors_obb.size(0), 5))
        labels = anchors_obb.new_full((num_valid_anchors, ),
                                  self.num_classes,
                                  dtype=torch.long)
        label_weights = anchors_obb.new_zeros(num_valid_anchors, dtype=torch.float)

        pos_inds = sampling_result.pos_inds
        neg_inds = sampling_result.neg_inds
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                # print('sampling_result.pos_bboxes: ', sampling_result.pos_bboxes.size())
                # print('sampling_result.pos_gt_bboxes: ', sampling_result.pos_gt_bboxes.size())
    ...
    def get_bboxes(self,
        ...
        # mlvl_anchors_old = self.anchor_generator.grid_priors(
        #     featmap_sizes, device=device)
        
        # for mlvl_anchor in mlvl_anchors[0]:
        #     print('mlvl_anchor: ', mlvl_anchor.size())
        # for mlvl_anchor_old in mlvl_anchors_old:
        #     print('mlvl_anchor_old: ', mlvl_anchor_old.size())
    ...
    def forward_train(self,
        ...
        outs = self(x)
        if gt_labels is None:
            loss_inputs = outs + (anchors, gt_bboxes, img_metas)
        else:
        ...
        if proposal_cfg is None:
            ...
            proposal_list = self.get_bboxes(
                *outs, img_metas=img_metas, cfg=proposal_cfg, mlvl_anchors=anchors)
    ...
    def loss(self,
        ...
        # for refined_anchor, anchor in zip(refined_anchor_list[0], anchor_list[0]):
        #     print('refined_anchor: ', refined_anchor.size())
        ...
        return dict(loss_rrnCls=losses_cls, loss_rrnBbox=losses_bbox)
    
    def simple_test_rpn(self, refined_anchors, x, img_metas):
        """Test without augmentation, only for ``RPNHead`` and its variants,
        e.g., ``GARPNHead``, etc.
        ...
        """
        rpn_outs = self(x)
        proposal_list = self.get_bboxes(*rpn_outs, img_metas=img_metas, mlvl_anchors=refined_anchors)
        return proposal_list